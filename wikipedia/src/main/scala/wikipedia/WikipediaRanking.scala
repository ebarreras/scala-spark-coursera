...
object WikipediaRanking {
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    (index map {case (lang, articles) => (lang, articles.size) } ) sortBy(_._2, false) collect() toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    (for {
      article <- rdd
      lang <- langs if article.mentionsLanguage(lang)
    } yield (lang, article)) map { case (lang, article) => (lang, 1) } reduceByKey(_ + _) sortBy (-_._2) collect() toList
